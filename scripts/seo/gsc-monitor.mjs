...
// Monitoring: re-inspect the same canonical URL set captured in the baseline,
// so checkpoint diffs are apples-to-apples. Override with GSC_BASELINE env.
const BASELINE = process.env.GSC_BASELINE || 'docs/gsc-reports/baseline-2026-05-26.json';
const _base = JSON.parse(readFileSync(BASELINE, 'utf-8'));
const URLS_TO_INSPECT = (_base.results || []).map((r) => ({ pattern: r.pattern || r.url, url: r.url }));
