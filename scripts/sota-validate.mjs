...
const totalSteps = SKIP_BUILD ? 5 : 6;
...
header(2, totalSteps, 'i18n Parity — 4420 chaves × 4 locales');
...
header(3, totalSteps, 'Test Suite — Testes Completos');
...
header(4, totalSteps, 'SEO — Canonical URLs, Hreflang, Meta Tags');
run('seo:validate', 'node scripts/seo/validate-pre-deploy.mjs');
success('Validação SEO completa');

// ── STEP 5: Rich Results ────────────────────────────────────────────────────────

header(5, totalSteps, 'SEO — JSON-LD, DID, Rich Results');
...
if (!SKIP_BUILD) {
  header(6, totalSteps, 'Build — SSG Completo (next build)');
